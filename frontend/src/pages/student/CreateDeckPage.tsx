...
export default function CreateDeckPage() {
  ...
  const setCardImage = (uid: string, file: File, preview: string) =>
    setCards((p) => p.map((c) => (c.uid === uid ? { ...c, imageFile: file, imagePreview: preview } : c)));

  const clearCardImage = (uid: string) =>
    setCards((p) => p.map((c) => (c.uid === uid ? { ...c, imageFile: undefined, imagePreview: undefined } : c)));
    ...
    try {
      ...
      for (const [i, card] of valid.entries()) {
        const fc = await flashcardApi.create({
          front: card.front.trim() || "(empty)",
          back: card.back.trim() || "(empty)",
          cardType: "BASIC",
          itemType: "WORD",
          itemId: 0,
          isActive: true,
        ...
        if (card.imageFile && fc.id) {
          try {
            const attachment = await fileApi.upload(card.imageFile, "flashcard", fc.id, "imageUrl");
            await flashcardApi.update(String(fc.id), { imageUrl: attachment.url });
          } catch {
      ...
      navigate(practice ? `/library` : `/library`);
    } catch {
